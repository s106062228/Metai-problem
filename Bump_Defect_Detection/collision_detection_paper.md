...
Phase 3: Narrow Phase (Exact Geometric Test)
  Input:  Candidate triangle pair (t_a, t_b) ∈ Q
...
where *p₁, p₂, p₃* are large primes and *H* is the hash table size. This provides O(1) cell access and O(1) neighbor enumeration.
...
```
Input:  Triangle A = (V₀, V₁, V₂), Triangle B = (U₀, U₁, U₂)
Output: Boolean intersect, Segment S (if intersecting)

1. Compute plane π_A containing A: N_A = (V₁-V₀) × (V₂-V₀), d_A = -N_A · V₀
2. Compute signed distances of B's vertices to π_A:
     d_{U_i} = N_A · U_i + d_A,  i ∈ {0, 1, 2}
...
7. Project both triangles onto L, obtaining intervals [t₁, t₂] and [s₁, s₂]
...
**Prerequisite:** Non-convex meshes are pre-processed using V-HACD [9] to produce a set of approximate convex hulls *{C₁, C₂, ..., Cₖ}*. Decomposition parameters — maximum hull count, voxel resolution, and maximum vertices per hull — are tuned to balance approximation quality against runtime performance.

**GJK Phase:** For each pair of convex components *(Cᵢ, Cⱼ)*, the GJK algorithm iteratively constructs a simplex in the Minkowski difference *Cᵢ ⊖ Cⱼ*, determining whether the origin is contained within the difference (indicating intersection).
...
where *v₀, v₁, v₂* are the vertices of each triangle *t* in the intersection mesh, with consistent outward-facing normal orientation.
...
3. **Sweep-and-Prune (SAP) maintenance:** The sorted endpoint lists are nearly sorted after a single-object move. Insertion sort on a nearly-sorted list achieves O(n + s) where *s* is the number of swaps, typically *s* ≪ *n*.
...
At factory coordinate scales (object positions at 10⁴–10⁵ mm from origin), single-precision floating-point arithmetic introduces errors exceeding the detection threshold:
...
2. **Double-precision arithmetic (float64):** Provides precision to approximately 10⁻¹¹ mm at factory scales, sufficient for all practical clearance thresholds.
...
- Triangle vertex data: 10⁸ × 36 bytes ≈ 3.6 GB
...
**Table 5.** Global audit timing projection (10,000 objects, 10⁸ total triangles, 8-core CPU).
